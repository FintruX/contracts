...
contract FTXToken is StandardToken, Ownable {
    ...
    uint256 public token4Gas = 5*10**18;        // minimum FTX token to be transferred to make the gas worthwhile(avoid micro transfer), cannot be higher than minimal subscribed amount in crowd sale
    ...
    */
    function transfer(address _to, uint256 _value) public returns (bool) {
        ...
        if (_value < token4Gas) {
            return false;                                                           // do nothing if less than allowed minimum but do not fail
        }
        ...
        if (_value > 0 && _value > token4Gas) { // if the transfer is large enough and greater than zero, rebate gas
            ...
            if (_to.balance < minGas4Accts) {
                // reimburse gas in ETH to keep a minimal balance for next transaction, use send instead of transfer thus ignore failed rebate.
    ...
    */
    ...
    function setToken4Gas(uint newFTXAmount) public onlyOwner {
        require(newFTXAmount < 150*10**18); // 0.1 ether @ 1500/ether
